...
整个应用程序依赖三个部分，分别是由Node.js运行的Nuxt部分，数据库部分，和文件存储部分。
Nuxt部分由pm2工具启动，并且在后台运行；数据库和文件存储部分均使用systemd运行。
...
::alert{type="info" icon="lucide:info"}
  当环境变量更改或重新构建后，需要先删除所有相应的pm2进程，再重新启动pm2。
::
...
编辑好之后使用 `Ctrl+O` 保存（需要 `Enter` 确认），然后使用 `Ctrl+X` 退出。
...
```
上述文件中的 MINIO-USERNAME 和 MINIO-PASSWORD 需要替换为你想要设定的MINIO管理员用户名和密码。该用户名和密码稍后将用于登录 MinIO 的 Web 界面。注意，不是Linux用户名和密码。
